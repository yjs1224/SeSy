...
class TCModel(BertPreTrainedModel):
    def __init__(self, config, **kwargs):
        ...
        self.classifier = nn.Linear(self.size+self.hidden_size, self.num_labels)
        self.init_weights()
    ...
    def forward(self,input_ids,attention_mask,token_type_ids,
        ...
        clf_input_gat = self.pool_gat(gat_out.permute(0,2,1)).squeeze(-1)
        clf_input_fc = outputs[0]
        clf_input_fc = self.pool(clf_input_fc.permute(0, 2, 1)).squeeze(-1)
        clf_input = torch.cat([clf_input_gat,clf_input_fc], dim=-1)
