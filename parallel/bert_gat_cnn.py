...
class TCModel(BertPreTrainedModel):
    def __init__(self, config, **kwargs):
        ...
        self.relu = nn.ReLU()
        self.pool_cnn = nn.AdaptiveMaxPool1d(1)
        self.pool_gat = nn.AdaptiveMaxPool1d(1)
        self.classifier = nn.Linear(self.filter_num * len(self.filter_size) + self.size, self.num_labels)
        self.init_weights()
    ...
    def forward(self, input_ids, attention_mask, token_type_ids,
        ...
        for cnn in self.cnns:
            x_tmp = cnn(x)
            x_tmp = self.pool_cnn(x_tmp)
            x_tmp = self.relu(x_tmp)
            clf_input.append(x_tmp.squeeze(2))

        clf_input_cnn = torch.cat(clf_input, 1)
        clf_input_cnn = self.dropout(clf_input_cnn)

        clf_input = torch.cat([clf_input_gat,clf_input_cnn],dim=-1)
