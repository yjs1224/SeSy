...
class TCModel(BertPreTrainedModel):
    def __init__(self, config, **kwargs):
        ...
        self.classifier = nn.Linear(self.size*2*2+self.size, self.num_labels)
        self.pool_gat = nn.AdaptiveMaxPool1d(1)
    ...
    def forward(self, input_ids, attention_mask, token_type_ids,
        ...
        clf_input_gat = self.pool_gat(gat_out).squeeze(-1)
        ...
        clf_input_rnn = rnn_out_last_time_hidden.view(2, 2, x.shape[0], self.size).permute(1,0,2,3).permute(2,1,0,3)
        clf_input_rnn = clf_input_rnn.reshape((x.shape[0], 2*2*self.size))  # batch, seqlen*direction_num, feature_num

        clf_input = torch.cat([clf_input_gat, clf_input_rnn], dim=-1)
